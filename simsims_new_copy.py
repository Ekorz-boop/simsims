...
class World():
    ...
    def run(self):
        ...
        for barrack in self.places["Barrack"]:
            for worker in range(barrack.queue.qsize()):
                # If food is prioritized over products
                if self.optimal_from_place("Barn").resources_in_place() <= self.optimal_from_place("Warehouse").resources_in_place():
                    for field in self.transitions["Field"]:
                    ...
                    for factory in self.transitions["Factory"]:
                        # Optimize where it gets workers from
...
class Fields(Transition):
    ...
    def __init__(self, to_barrack, from_barrack, barn):
        self.barn = barn
        self.accident_risk = random.choice([1, 1, 1, 3, 5, 5, 10, 15])
        super().__init__(to_barrack, from_barrack)
    ...
    def produce(self):
        ...
        if worker is not None:
            ...
            if random.randint(1, 100) <= self.accident_risk:
                self.modify_worker_health(worker, random.randint(-15, -5))
...
class Canteen(Transition):
    ...
    def produce(self):
        ...
        if worker and food is not None:
            if food.quality >= 50:
...
if __name__ == "__main__":
    ...
    game_world.setup(2, 1, 2, 2, 2, 4, 3)
    # ui = UserInterface(game_world)
    # ui.show_state()
    cycle = 0
    running = True
    while running and cycle < 5:
